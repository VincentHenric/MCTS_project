# sudoku.py
# https://gist.github.com/MHenderson/7639387
...
def solve_puzzle(fixed, boxsize, model = 'CP', **kwargs):
    """Solve 'puzzle' exploiting 'model' ('lp', 'graph', 'CP', 'groebner')."""
    if model == 'CP':
        return solve_as_CP(fixed, boxsize, **kwargs)
#    elif model == 'lp':
...
def solve(puzzle, model = 'CP', **kwargs):
    fixed = puzzle.get_fixed()
    boxsize = puzzle.get_boxsize()
    return Puzzle(solve_puzzle(fixed, boxsize, model, **kwargs), boxsize)
